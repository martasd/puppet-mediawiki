...
Vagrant.configure("2") do |config|
  ...
  config.vm.synced_folder ".", "/etc/puppet/modules/mediawiki"
  ...
  [ 'puppetlabs/stdlib', 'puppetlabs/apache', 'puppetlabs/mysql', 'saz/memcached' ].each do |mod|
    config.vm.provision "shell",
      inline: "puppet module install #{mod} --modulepath=/usr/share/puppet/modules"
  end
